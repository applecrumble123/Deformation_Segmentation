...
from modules_bisenetv2 import (conv3x3, conv1x1, DWConvBNAct, PWConvBNAct, ConvBNAct, 
                        Activation, SegHead)
...
class SemanticBranch(nn.Sequential):
    ...
    def forward(self, x):
        #print("x: ", x.shape)
        x = self.stage1to2(x)
        #print("x_stage1to2: ", x.shape)
        if self.use_aux:
            aux2 = self.seg_head2(x)
            #print("aux2: ", aux2.shape)
        ...
        if self.use_aux:
            ...
            #print("aux5: ", aux5.shape)


        x = self.stage5_5(x)
        #print("x_stage5_5: ", x.shape)
...
class StemBlock(nn.Module):
    def __init__(self, in_channels, out_channels, act_type='relu'):
        ...
        self.conv_init = ConvBNAct(in_channels, out_channels, 3, 1, act_type=act_type)
    ...
    def forward(self, x):
        #print("stem block original x: ", x.shape)
        x = self.conv_init(x)
        #print("stem block conv_init x: ", x.shape)
        x_left = self.left_branch(x)
        #print("stem block left_branch x: ", x_left.shape)
        x_right = self.right_branch(x)
        #print("stem block right_branch x: ", x_right.shape)
        x = torch.cat([x_left, x_right], dim=1)
        #print("stem block cat x: ", x_right.shape)
        x = self.conv_last(x)
...
class BilateralGuidedAggregationLayer(nn.Module):
    def __init__(self, in_channels, out_channels, act_type='relu'):
        ...
        self.detail_low = nn.Sequential(
                                    DWConvBNAct(in_channels, in_channels, 3, 1, act_type=act_type),
                                    nn.AvgPool2d(3, 2, 1)
                        )
    ...
    def forward(self, x_d, x_s):
        ...
        # print("x_d: ",x_d.shape)
        # print("x_d_high: ", x_d_high.shape)
        # print("x_s: ", x_s.shape)
        # print("x_s_high: ", x_s_high.shape)

        # print("x_d: ",x_d.shape)
        # print("x_d_low: ", x_d_low.shape)
        # print("x_s: ", x_s.shape)
        # print("x_s_low: ", x_s_low.shape)
