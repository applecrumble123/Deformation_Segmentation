...
from models.lednet import Encoder, Decoder, Net
...
class DeformSegmentationModule(SegmentationModuleBase):
    ...
    def __init__(self, segmentation_model, net_saliency, net_compress, crit, cfg, deep_sup_scale=None):
    ...
    def forward(self, feed_dict, *, writer=None, segSize=None, F_Xlr_acc_map=False, count=None, epoch=None, feed_dict_info=None, feed_batch_count=None):
        ...
        if segSize is None:
            ...
            if self.deep_sup_scale is not None: # use deep supervision technique
                ...
                #pred = self.decoder(self.encoder(x_sampled))
                pred = self.segmentation_model(x_sampled)
            ...
            if self.cfg.MODEL.loss_at_high_res:
                ...
                #print("feed_dict['seg_label']: ", feed_dict['seg_label'])
        ...
        # EXPLAIN: if inference
            ...
            #x = self.decoder(self.encoder(x_sampled))
            x = self.segmentation_model(x_sampled)
...
class ModelBuilder:
    ...
    #     elif arch == 'lednet_encoder':
    #         net_encoder = Encoder(num_classes=30)
    #     else:
    ...
    #             use_softmax=use_softmax,
    #             fpn_dim=512)
    #     elif arch == 'lednet_decoder':
    #         net_decoder = Decoder(num_classes=30)
    #     else:
    #         raise Exception('Architecture undefined!')
    ...
    @staticmethod
    #num_class=150
    def build_model(num_class=150, weights=''):
        model = BiSeNetv2(num_class=num_class)
        if len(weights) > 0:
            print('Loading weights for net_decoder')
